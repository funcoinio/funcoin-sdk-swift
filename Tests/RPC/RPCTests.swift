//
//  RPCTests.swift
//  FUNCOINTests
//
//  Created by Yate Fulham on 2018/08/10.
//  Copyright © 2018 Cryptape. All rights reserved.
//

import XCTest
import BigInt
@testable import FUNCOIN

class RPCTests: XCTestCase {
    private static var txHashRecentSent: String!

    override class func setUp() {
        super.setUp()

        do {
            let funcoin = FUNCOIN.default
            let currentBlock = try funcoin.rpc.blockNumber()
            let metaData = try funcoin.rpc.getMetaData()

            let privateKey = "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
            let tx = Transaction(
                to: Address("0x0000000000000000000000000000000000000000"),
                nonce: UUID().uuidString,
                validUntilBlock: currentBlock + 88,
                chainId: metaData.chainId,
                version: metaData.version
            )
            let signed = try! Signer().sign(transaction: tx, with: privateKey)

            txHashRecentSent = try funcoin.rpc.sendRawTransaction(signedTx: signed)
        } catch {
            txHashRecentSent = ""
        }

        Thread.sleep(forTimeInterval: 10) // Allow it to be mined
    }

    func testPeerCount() throws {
        let peerCount = try funcoin.rpc.peerCount()
        XCTAssertTrue(peerCount > 0)
    }

    func skip_testPeerInfo() throws {
        let peersInfo = try funcoin.rpc.peersInfo()
        XCTAssertTrue(peersInfo.amount > 0)
        XCTAssertEqual(Int(peersInfo.amount), peersInfo.peers.count)
    }

    func testBlockNumber() throws {
        let blockNumber = try funcoin.rpc.blockNumber()
        XCTAssertTrue(blockNumber > 100)
    }

    func testSendRawTransaction() throws {
        let currentBlock = try funcoin.rpc.blockNumber()
        let metaData = try funcoin.rpc.getMetaData()

        let privateKey = "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
        let tx = Transaction(
            to: Address("0x0000000000000000000000000000000000000000"),
            nonce: UUID().uuidString,
            validUntilBlock: currentBlock + 88,
            data: Data.fromHex("")!,
            chainId: metaData.chainId,
            version: metaData.version
        )
        guard let signed = try? Signer().sign(transaction: tx, with: privateKey) else {
            return XCTFail("Sign tx failed")
        }

        let txhash = try funcoin.rpc.sendRawTransaction(signedTx: signed)
        XCTAssert(txhash.hasPrefix("0x"))
        XCTAssertEqual(66, txhash.count)
    }

    func skip_testGetVersion() throws {
        let version = try funcoin.rpc.getVersion()
        XCTAssertNotNil(version.softwareVersion)
    }

    func testGetBlockByHash() throws {
        let block = try funcoin.rpc.getBlockByNumber(number: BigUInt(try funcoin.rpc.blockNumber()), fullTransactions: true)
        let hash = block.hash.toHexString().addHexPrefix()
        let query = try funcoin.rpc.getBlockByHash(hash: hash, fullTransactions: true)
        XCTAssertEqual(query.hash.toHexString().addHexPrefix(), hash)
    }

    func testGetBlockByNumberNullProof() throws {
        let number = BigUInt(0)
        let block = try funcoin.rpc.getBlockByNumber(number: number, fullTransactions: true)
        XCTAssertEqual(block.header.number, number)
    }

    func testGetBlockByNumber() throws {
        let currentBlock = try funcoin.rpc.blockNumber()
        let block = try funcoin.rpc.getBlockByNumber(number: BigUInt(currentBlock), fullTransactions: true)
        XCTAssertEqual(block.header.number, BigUInt(currentBlock))
    }

    func testGetTransactionReceipt() throws {
        let receipt = try funcoin.rpc.getTransactionReceipt(txhash: RPCTests.txHashRecentSent!)
        XCTAssertEqual(receipt.transactionHash.toHexString().addHexPrefix(), RPCTests.txHashRecentSent!)
    }

    func testGetLogs() throws {
        var filter = Filter()
        filter.fromBlock = "0x0"
        filter.topics = [["0xe4af93ca7e370881e6f1b57272e42a3d851d3cc6d951b4f4d2e7a963914468a2", "0xa84557f35aab907f9be7974487619dd4c05be1430bf704d0c274a7b3efa50d5a", "0x00000000000000000000000000000000000000000000000000000165365f092d"]]
        let logs = try funcoin.rpc.getLogs(filter: filter)
        XCTAssertTrue(logs.count >= 0)
    }

    func testCall() throws {
        let request = CallRequest(from: "0x46a23e25df9a0f6c18729dda9ad1af3b6a131160", to: "0x6fc32e7bdcb8040c4f587c3e9e6cfcee4025ea58", data: "0x9507d39a000000000000000000000000000000000000000000000000000001653656eae7")
        let data = try funcoin.rpc.call(request: request)
        XCTAssert(data.hasPrefix("0x"))
    }

    func testGetTransaction() throws {
        let txhash = RPCTests.txHashRecentSent!
        let tx = try funcoin.rpc.getTransaction(txhash: txhash)
        XCTAssertEqual(tx.hash.toHexString().addHexPrefix(), txhash)
        XCTAssertEqual(tx.unsignedTransaction?.sender.address, "0x46a23e25df9a0f6c18729dda9ad1af3b6a131160")
    }

    func testGetTransactionCountByBlockNumber() throws {
        let count = try funcoin.rpc.getTransactionCount(address: "0x4b5ae4567ad5d9fb92bc9afd6a657e6fa13a2523", blockNumber: "0xf5dc9")
        XCTAssert(count >= 0)
    }

    func testGetTransactionCountLatest() throws {
        let count = try funcoin.rpc.getTransactionCount(address: "0x4b5ae4567ad5d9fb92bc9afd6a657e6fa13a2523", blockNumber: "latest")
        XCTAssertTrue(count >= 0)
    }

    func testGetTransactionCountByAddress() throws {
        let address = Address("0x4b5ae4567ad5d9fb92bc9afd6a657e6fa13a2523")!
        let count = try funcoin.rpc.getTransactionCount(address: address, blockNumber: "latest")
        XCTAssertTrue(count >= 0)
    }

    func testGetCode() throws {
        let code = try funcoin.rpc.getCode(address: "0xd8fb3e5600a682f340761280ccf9d29c7ee114a7", blockNumber: "latest")
        XCTAssert(code.hasPrefix("0x"))
    }

    func testGetAbi() throws {
        let code = try funcoin.rpc.getAbi(address: "0xb93b22a67D724A3487C2BD83a4aaac66F1B7C882", blockNumber: "latest")
        XCTAssert(code.hasPrefix("0x"))
    }

    func testGetBalance() throws {
        let balance = try funcoin.rpc.getBalance(address: "0x4b5ae4567ad5d9fb92bc9afd6a657e6fa13a2523", blockNumber: "0xf5dc9")
        XCTAssertTrue(balance >= 0)
    }

    func testNewFilter() throws {
        var filter = Filter()
        filter.fromBlock = "0x0"
        filter.topics = [["0xe4af93ca7e370881e6f1b57272e42a3d851d3cc6d951b4f4d2e7a963914468a2", "0xa84557f35aab907f9be7974487619dd4c05be1430bf704d0c274a7b3efa50d5a", "0x00000000000000000000000000000000000000000000000000000165365f092d"]]
        let id = try funcoin.rpc.newFilter(filter: filter)
        XCTAssertTrue(id > 0)
    }

    func testNewBlockFilter() throws {
        let id = try funcoin.rpc.newBlockFilter()
        XCTAssertTrue(id > 0)
    }

    func testUninstallFilter() throws {
        let filterID = try funcoin.rpc.newBlockFilter()
        let uninstalled = try funcoin.rpc.uninstallFilter(filterID: filterID)
        XCTAssertTrue(uninstalled)

        let uninstalledAgain = try funcoin.rpc.uninstallFilter(filterID: filterID)
        XCTAssertFalse(uninstalledAgain)
    }

    func testGetFilterChanges() throws {
        let changes = try funcoin.rpc.getFilterChanges(filterID: 1)
        XCTAssertTrue(changes.count >= 0)
    }

    func testGetFilterLogs() throws {
        let changes = try funcoin.rpc.getFilterLogs(filterID: 1)
        XCTAssertTrue(changes.count >= 0)
    }

    func testGetTransactionProof() throws {
        let proof = try funcoin.rpc.getTransactionProof(txhash: RPCTests.txHashRecentSent!)
        XCTAssert(proof.hasPrefix("0x"))
    }

    func testGetMetaData() throws {
        let metaData = try funcoin.rpc.getMetaData(blockNumber: "latest")
        XCTAssertEqual(metaData.chainName, "test-chain")
    }

    func testGetBlockHeader() throws {
        let blockNumber = try funcoin.rpc.blockNumber()
        let blockHeader = try funcoin.rpc.getBlockHeader(blockNumber: blockNumber - 1)
        XCTAssert(blockHeader.hasPrefix("0x"))
    }

    func xtestGetStateProof() throws {
        let proof = try funcoin.rpc.getStateProof(address: "0xad54ae137c6c39fa413fa1da7db6463e3ae45664", key: "0xa40893b0c723e74515c3164afb5b2a310dd5854fac8823bfbffa1d912e98423e", blockNumber: "latest")
        XCTAssertEqual(proof, "")
    }
}

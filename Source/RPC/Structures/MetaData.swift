...
public struct MetaData: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard let genesisTimestamp = try DecodeUtils.decodeIntToBigUInt(container, key: .genesisTimestamp) else { throw FUNCOINError.dataError }
        self.genesisTimestamp = genesisTimestamp

        self.validators = try container.decode([String].self, forKey: .validators)

        guard let blockInterval = try DecodeUtils.decodeIntToBigUInt(container, key: .blockInterval) else { throw FUNCOINError.dataError }

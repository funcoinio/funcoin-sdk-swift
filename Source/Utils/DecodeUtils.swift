...
struct DecodeUtils {
    static func decodeHexToBigUInt<T>(_ container: KeyedDecodingContainer<T>, key: KeyedDecodingContainer<T>.Key, allowOptional: Bool = false) throws -> BigUInt? {
        if allowOptional {
            ...
            if string != nil {
                guard let number = BigUInt(string!.stripHexPrefix(), radix: 16) else { throw FUNCOINError.dataError }
                return number
            }
            return nil
        } else {
    ...
    static func decodeHexToData<T>(_ container: KeyedDecodingContainer<T>, key: KeyedDecodingContainer<T>.Key, allowOptional: Bool = false) throws -> Data? {
        if allowOptional {
            let string = try? container.decode(String.self, forKey: key)
            if string != nil {
                guard let data = Data.fromHex(string!) else { throw FUNCOINError.dataError }
                return data
            }
            return nil
        } else {
